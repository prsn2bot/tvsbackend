...
describe("PdfExtractorService", () => {
    ...
    beforeEach(() => {
        pdfExtractor = new pdfExtractor_service_1.PdfExtractorService();
        jest.clearAllMocks();
    });
    describe("extractText", () => {
        ...
        it("should throw error for non-existent file", async () => {
            ...
            await expect(pdfExtractor.extractText(mockFilePath)).rejects.toThrow("File not found");
        });
        it("should throw error for file too large", async () => {
            ...
            fs_1.default.promises.stat.mockResolvedValue(mockStats);
            await expect(pdfExtractor.extractText(mockFilePath)).rejects.toThrow("File too large");
        });
        it("should throw error for unsupported file format", async () => {
            const txtFilePath = "/test/document.txt";
            const mockStats = { isFile: () => true, size: 1024 };
            fs_1.default.promises.stat.mockResolvedValue(mockStats);
            await expect(pdfExtractor.extractText(txtFilePath)).rejects.toThrow("Unsupported file format");
        });
        it("should handle corrupted PDF file", async () => {
            const mockStats = { isFile: () => true, size: 1024 };
            fs_1.default.promises.stat.mockResolvedValue(mockStats);
            fs_1.default.promises.readFile.mockResolvedValue(Buffer.from("invalid pdf data"));
            const mockLoadingTask = {
                promise: Promise.reject(new Error("Invalid PDF structure")),
            };
            const pdfjs = require("pdfjs-dist");
            pdfjs.getDocument = jest.fn().mockReturnValue(mockLoadingTask);
            await expect(pdfExtractor.extractText(mockFilePath)).rejects.toThrow("Invalid PDF structure");
        });
    });
...
//# sourceMappingURL=pdfExtractor.test.js.map