...
class CaseModel {
     ...
     */
    static async findDocumentsByCaseId(caseId) {
     ...
     * @returns Promise<Case | null> - The updated case or null if not found
     */
    static async assignCVO(caseId, cvoId) {
        const query = `
      UPDATE cases
      ...
      RETURNING *;
    `;
        const result = await database_1.pool.query(query, [cvoId, caseId]);
        if (result.rows.length === 0)
            return null;
        const row = result.rows[0];
        return {
     ...
     * @returns Promise<Case | null> - The updated case or null if not found
     */
    static async assignLegalBoard(caseId, legalBoardId) {
        const query = `
      UPDATE cases
      ...
      RETURNING *;
    `;
        const result = await database_1.pool.query(query, [legalBoardId, caseId]);
        if (result.rows.length === 0)
            return null;
        const row = result.rows[0];
        return {
     ...
     */
    static async findCasesByAssignedUser(userId, userRole) {
        let query;
        let params;
        if (userRole === "cvo") {
