...
const aiDraftProcessor = async (job) => {
    ...
    try {
        ...
        const latestDraft = await aiDraft_model_1.AiDraftModel.findLatestByCaseId(document.case_id.toString());
        const nextVersion = latestDraft ? latestDraft.version + 1 : 1;
        logger_1.default.info(`Creating AI draft version ${nextVersion} for case ${document.case_id}`);
