import { PdfToImageConverter } from "../pdfToImageConverter.service";
...
describe("PdfToImageConverter", () => {
  ...
  describe("convertPdfToImages", () => {
    ...
    it("should throw error for file too large", async () => {
      ...
      await expect(converter.convertPdfToImages(mockFilePath)).rejects.toThrow(
        "File too large"
      );
    });
    ...
    it("should throw error for unsupported file format", async () => {
      ...
      await expect(converter.convertPdfToImages(txtFilePath)).rejects.toThrow(
  ...
  describe("convertAndSaveImages", () => {
    it("should convert and save images to disk", async () => {
      ...
      const result = await converter.convertAndSaveImages(
        mockFilePath,
        mockOutputDir,
        { filenamePrefix: "test_doc" }
      );
    ...
    it("should create output directory if requested", async () => {
      ...
      expect(fs.promises.mkdir).toHaveBeenCalledWith(mockOutputDir, {
        recursive: true,
      });
    });
  });
  ...
  describe("convertPagesStreaming", () => {
    it("should convert specific pages only", async () => {
      ...
      const result = await converter.convertPagesStreaming(
        mockFilePath,
        [2, 5, 8]
      );
    ...
    it("should skip invalid page numbers", async () => {
      ...
      const result = await converter.convertPagesStreaming(
        mockFilePath,
        [0, 3, 10] // 0 and 10 are invalid for a 5-page document
      );
