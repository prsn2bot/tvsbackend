...
import { ocrMetadataService } from "../ocrMetadata.service";
import fs from "fs";
...
describe("OCR System Integration Tests", () => {
  ...
  describe("End-to-End OCR Processing", () => {
    it("should process PDF document with native text extraction", async () => {
      ...
      const result = await ocrOrchestrator.extractText(
        "/test/legal-document.pdf"
      );
    ...
    it("should use Cloudinary fallback when other methods fail", async () => {
      ...
      const result = await ocrOrchestrator.extractText(
        "documents/fallback_test_123"
      );

      expect(result.text).toBe(
        "Cloudinary extracted text from fallback service"
      );
  ...
  describe("Batch Processing", () => {
    it("should process multiple documents with mixed results", async () => {
      ...
      const results = await ocrOrchestrator.processDocumentsBatch(documentUrls);
  ...
  describe("Error Handling and Recovery", () => {
    it("should handle configuration errors gracefully", async () => {
      ...
      (ocrConfig.validateConfiguration as jest.Mock).mockReturnValue(false);

      await expect(
        ocrOrchestrator.extractText("/test/document.pdf")
      ).rejects.toThrow("Invalid OCR configuration");
    });
    ...
    it("should handle file system errors", async () => {
      ...
      await expect(
        ocrOrchestrator.extractText("/nonexistent/document.pdf")
      ).rejects.toThrow();
    });
    ...
    it("should handle all methods failing", async () => {
      ...
      await expect(
        ocrOrchestrator.extractText("/test/problematic-document.pdf")
